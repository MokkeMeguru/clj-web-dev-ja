  ...
  (let [[image-urls err] (pic-post-try-phase-save-images (:image-files input-model) image-db)
        [new-pic err] (if err
                        ...
                        (pic-post-try-phase-save-pic {:input-model input-model
                                                      ...
                                                      :image-urls image-urls}
                                                     db))]
    (timbre/info "pic-post tcc try phase result: save-images" image-urls)
    (timbre/info "pic-post tcc try phase result: save-pic" new-pic)
    (timbre/info "pic-post tcc try phase result: error" err)
    (if err
...
他実装は signin / signup と同様の記述であり、複雑な内容がないため、紹介を省略します。
