...
* [`src/clj/pic_gallery_web/`](src/clj/pic_gallery_web/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/pic_gallery_web/`](src/cljs/pic_gallery_web/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/pic_gallery_web/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/pic_gallery_web/`](test/cljs/pic_gallery_web/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `pic-gallery-web` project root directory
...
in the [`src/clj/pic_gallery_web/`](src/clj/pic_gallery_web/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/pic_gallery_web/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns pic-gallery-web.example
  (:require [pic-gallery-web.config :as config])
